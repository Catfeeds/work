...
/*     	$arryReturn = [];
     ...
     * @return array 接口数据
     */
    public function getUserFirstOrderList($param)
    {
    	if (empty($param['suppliers_id']) ) {
    		baf_Logger::log2File(__METHOD__.'参数出错' . json_encode($param) , 'Orders_api_error');
    	}
    
    	$path = 'shangjia/order/getUserFirstOrderStatInfo';
    	$params = [
     ...
     * @return array 接口数据
     */
    public function getOrderSettlePriceByDate($param)
    {
    	if (empty($param['venues_id']) ) {
    		baf_Logger::log2File(__METHOD__.'参数出错' . json_encode($param) , 'Orders_api_error');
    	}
    
    	$path = 'shangjia/bill/statistic_category_by_date_range';
    	$params = [
    			'venues_id'		=> $param['venues_id'],
    			'start_time'	=> $param['start_time'],
    			'end_time'		=> $param['end_time'],

...
class GoodslistController extends BaseController
{
	...
	public function changeGoodsAction(){
		...
		//检查当前的状态
		$goods_ids = [];
		foreach ($data_list[0] as $c){
			if(!isset($goods_ids[$c['goods_id']])){
				$goods_ids[$c['goods_id']] = $c['status'];
			}else{
			    if($goods_ids[$c['goods_id']] < $c['status']){
			        $goods_ids[$c['goods_id']] = $c['status'];
			    }
			}
		}
		foreach ($data_list[1] as $c){ 
		    if(!isset($goods_ids[$c['goods_id']])){
		        $goods_ids[$c['goods_id']] = $c['status'];
		    }else{
		        if($goods_ids[$c['goods_id']] < $c['status']){
		...
		baf_Logger::log2File('VenuesId:'.$venues_id.';cat_id:'.$cat_id.';goods_ids:'.json_encode($goods_ids), 'checkGoodsStatus_info');
		if(!empty($goods_ids)){
			$param = [
					'venues_id'	=>	$venues_id,
					'cat_id'	=>	$cat_id,
					'book_date'	=>	strtotime($book_date),
					'goods_ids'	=>	implode(",", array_keys($goods_ids))
			];
			$checkGoodsStatus = Loader::api('Venues')->getGoodsStatusList($param);

			baf_Logger::log2File('VenuesId:'.$venues_id.';cat_id:'.$cat_id.';checkGoodsStatus:param'.json_encode($param).':'.json_encode($checkGoodsStatus), 'GoodsListChang_info');

			if(count($checkGoodsStatus) >0){
				foreach ($checkGoodsStatus as $c_goods){
					if(isset($goods_ids[$c_goods['goods_id']]) && $goods_ids[$c_goods['goods_id']] != $c_goods['status']){
						$this->errorOutput(baf_ErrorCode::ERR_UNKNOWN, "所选场地的状态已变换，请重新换场");
						exit;
					}
				}
			}			
		...
		baf_Logger::log2File('VenuesId:'.$venues_id.';cat_id:'.$cat_id.';goods_repeat:'.json_encode($goods_repeat), 'GoodsListChang_info');
		...
		foreach ($data_list[0] as $key=>$value){
			...
			if(empty($goods_repeat)){
				...
				//订单换锁场
					...
					$goods_id_lock_list[$temp][] = $value;
					$order_list[$temp_after][] = $list_new[$key];
					baf_Logger::log2File('VenuesId:'.$venues_id.';cat_id:'.$cat_id.';goods_id_lock_list:'.json_encode($goods_id_lock_list), 'GoodsListChang_info');
					//涉及订单与锁场互换，锁场信息中间表需要直接删掉
		...
		if(!empty($order_info)){
				...
				baf_Logger::log2File('VenuesId:'.$venues_id.';cat_id:'.$cat_id.';replaceOrderGoods:'.json_encode($param).', result:'. json_encode($info), 'GoodsListChang_info');
	...
	public function bookGoodsAction()
	{
		...
		//检查当前的状态
		 /* if(!empty($goods_ids)){
			...
			baf_Logger::log2File('VenuesId:'.$venues_id.';cat_id:'.$cat_id.';checkGoodsStatus:'.json_encode($checkGoodsStatus), 'GoodsListChang_info');
		
			if(count($checkGoodsStatus) >0){
				foreach ($checkGoodsStatus as $c_goods){
					if(isset($goods_ids[$c_goods['goods_id']]) && $goods_ids[$c_goods['goods_id']] != $c_goods['status']){
						$this->errorOutput(baf_ErrorCode::ERR_UNKNOWN, "所选场地的状态已变换");
						exit;
					}
				}
			}
		...
		if(!empty($data_list)){
			...
			baf_Logger::log2File('VenuesId:'.$venues_id.';cat_id:'.$cat_id.';data:'.json_encode($param), 'GoodsListLocked_info');
	...
	public function bookGoodsLaterAction()
	{
		...
		if(!empty($data_list)){
			...
			$param['name'] 			= $name;
			$param['phone']			= $phone;
			baf_Logger::log2File('later:VenuesId:'.$venues_id.';cat_id:'.$cat_id.';data:'.json_encode($param), 'GoodsListLocked_info');
	...
	public function unBookGoodsInfoAction()
	{
		...
		if(!empty($data_list)){
			...
			$param['book_date']		= $book_date;
			$param['utm_medium']	= helper_VenuesHelper::getutmSource();//操作请求来源
			baf_Logger::log2File('VenuesId:'.$venues_id.';cat_id:'.$cat_id.';data:'.json_encode($param), 'GoodsListUnLocked_info');
	...
	public function unBookGoodsLaterAction()
	{
		...
		if(!empty($data_list)){
			...
			baf_Logger::log2File('later:VenuesId:'.$venues_id.';cat_id:'.$cat_id.';data:'.json_encode($param), 'GoodsListUnLocked_info');

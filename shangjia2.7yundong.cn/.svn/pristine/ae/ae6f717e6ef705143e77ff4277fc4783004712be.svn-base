...
class StatController extends BaseController
{
    ...
    public function exportGoodsIncomeAction()
    {
        try {
            ...
            $phpExcel->getActiveSheet()->getColumnDimension('H')->setWidth(11);// 设置宽度
            ...
            $sheet->setCellValue("D{$index}", '单价');
            $sheet->setCellValue("E{$index}", '现金收入');
            $sheet->setCellValue("F{$index}", '会员收入');
            $sheet->setCellValue("G{$index}", '其他收入');
            $sheet->setCellValue("H{$index}", '销售额(元)');
            $sheet->setCellValue("I{$index}", '盈利');
            $sheet->setCellValue("J{$index}", '库存');
            ...
            foreach ($list as $key => $val) {
                ...
                $sheet->setCellValue("E{$index}", $val['cash_price']);
                $sheet->setCellValue("F{$index}", $val['member_card_price']);
                $sheet->setCellValue("G{$index}", $val['other_price']);
                $sheet->setCellValue("H{$index}", $val['sale_price']);
                $sheet->setCellValue("I{$index}", $val['profit']);
                $sheet->setCellValue("J{$index}", $val['stock_count']);

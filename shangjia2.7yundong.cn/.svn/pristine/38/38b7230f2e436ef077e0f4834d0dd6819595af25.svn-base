...
class ReportController extends BaseController
{
     ...
     *
     */
    public function userOrderedStatAction(){
    	$search = [];
    	 
    	// 商家id
    	$suppliersId = helper_LoginHelper::getCurrentSuppliersId(); 
    	$defalut_time = date("Y-m-d", mktime(0,0,0,date("m")-1,date("d"),date("Y")));
    	$search['start_date'] = trim($this->getParam('start_date', $defalut_time));  // 开始日期
    	$search['end_date']   = trim($this->getParam('end_date', date("Y-m-d", time())));    // 结束日期
    	$start_time = strtotime($search['start_date']);
    	...
    	$re = Loader::api('Orders')->getUserFirstOrderList($params);
    	
    	if(!empty($re) && isset($re['status']) && $re['status'] == "0000" && isset($re['data'])){
    		$orderList = $re['data'];
    	}
    	//分页
    	$page = array(
    			'totalPage'		=>	$orderList['total_page'],
    			'currentPage'	=>	$currentPage,
    			'url'			=>	'/report/userOrderedStat?'.http_build_query(array_filter($search)).'&page='
    	);    	
    	
    	$this->setTitle('新增人气度');
    	$this->display('userOrderedStat', [
    			'userOrderedList'	=>	$orderList['list'],
    			'search' 			=>	$search,
    			'page' 				=>	$page,
     ...
     *
     */
    public function userOrderedStatAddAction(){
        $search = [];
        ...
        $page = array(
                'totalPage'     =>  $orderList['total_page'],
                'currentPage'   =>  $currentPage,
                'url'           =>  '/report/userOrderedStatAdd?'.http_build_query(array_filter($search)).'&page='
        );      
        
        $this->setTitle('新交易用户数');
        $this->display('userOrderedStatAdd', [
                'userOrderedList'   =>  $orderList['list'],
                'search'            =>  $search,
                'page'              =>  $page,
                'total'             =>  $orderList['total_count'],

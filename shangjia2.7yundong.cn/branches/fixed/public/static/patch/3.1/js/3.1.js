    ...
    var mouseFn = {
        ...
        setTimeoutAjax:function(){
            ...
            this.timer = setTimeout(function(){
                if (main.snapshotData[0]) { that.ajaxExChange(); }
            },this.TIME);
        },
        ...
        compareData:function(){
            ...
            $.each(changeDataList,function(index,item){
                ...
                if (testArray.indexOf(dualWayCode)==-1&&item.status!="0") {
                    testArray.push(dualWayCode);
                    reform.push(item);
                }
            });
            for(var l=0;l<changeDataList.length/2;l++){
                if (changeDataList[l].status=="0"&&changeDataList.length/2==reform.length) { exPosition();break; }
            }

            function exPosition() {
                var exArray = [];
                for (var k = 0; k < changeDataList.length; k++) {
                    k<changeDataList.length/2?exArray.push(changeDataList[changeDataList.length/2+k]):exArray.push(changeDataList[k-changeDataList.length/2]);
                }
                changeDataList = exArray;
            }

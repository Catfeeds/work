...
class helper_VenuesHelper
{
    ...
    public static function getShowConfig($supplier_id){
    	...
    	return empty($showConfig) ? ["is_show_name"=> 1, "is_merge"=> 0] : $showConfig;
    }
     ...
     *
     */
    public  static function checkSomsVer()
    {
    	$list = [];
    	$venues_id = self::getCurrentVenuesId();
    	...
    	if(!empty($venues_id)){
	    	 $redis = baf_Redis::factory();
	    	 $cacheKey = 'soms_ver:' . '-' . $venues_id;
	    	 $list = $redis->getArrayValue($cacheKey);
    	 }
    	 
    	if (empty($list)) {
    		//场馆列表
    		$list = Loader::api('Stat')->getVersionInfo(["venues_id"=>$venues_id]);
    		
    		if (!empty($list)) {
    		 	$redis->setArrayValue($cacheKey, $list, 600);
    		}
    	}
    	return !empty($list['soms_version']) && floatval($list['soms_version']) >= self::SOMS_VER ? true : false;
    }

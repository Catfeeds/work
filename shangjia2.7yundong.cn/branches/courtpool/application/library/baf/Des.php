...
class baf_Des{
    ...
    public static function encrypt($input, $desKey='', $desIv='')
    {
        $size = mcrypt_get_block_size(MCRYPT_DES, MCRYPT_MODE_CBC); // 3DES加密将MCRYPT_DES改为MCRYPT_3DES
         
        $input = self::pkcs5Pad($input, $size); // 如果采用PaddingPKCS7，请更换成PaddingPKCS7方法。
        $desKey = $desKey ? $desKey : self::KEY;
        $key = str_pad($desKey, 8, '0'); // 3DES加密将8改为24
        $td = mcrypt_module_open(MCRYPT_DES, '', MCRYPT_MODE_CBC, '');
        $desIv = $desIv ? $desIv : self::IV;
        if ($desIv == '') {
    ...
    public static function decrypt($encrypted, $desKey='', $desIv='')
    {

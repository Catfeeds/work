...
class baf_Mysql{
    ...
    public static function factory($server = 'master', array $dbConfig)
    {
        ...
        if (!empty($existInstance) && ($existInstance instanceof PDO)) {
            try {
                ...
                if ($existInstance->errorCode() > 0) {
                    ...
                    //for---log  
                    baf_Logger::log2File('pdo_err_1'.json_encode($logArr), 'db_error');
                } else {
                ...
                //for---log                
                baf_Logger::log2File('pdo_err_2'.json_encode($logArr), 'db_error');
            }
        }
        ...
        try {
            ...
            //for---log  
            baf_Logger::log2File('pdo_err_3'.json_encode($logArr), 'db_error');
            ...
            if(in_array($e->getCode(), array(2002,2003,2006,2013))){
                try{
                    ...
                    baf_Logger::log2File('pdo_error_again'.json_encode($logArr), 'db_error');

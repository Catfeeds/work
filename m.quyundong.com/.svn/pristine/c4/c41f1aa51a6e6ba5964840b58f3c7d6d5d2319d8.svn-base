...
class UserController extends UserBaseController
{
	...
	public function getDeliveryAddressAction(){
		$userId = $this->uid;
		$res = api_User::deliveryAddress($userId);
		if(isset($res['status']) && $res['status']=='0000'){
			$this->renderJson($res);
		}else{
	 ...
	 * 添加收货地址
	 */
	public function addDeliverAddressAction(){
		$param = array(
				'user_id' => $this->uid,
				'consignee' => trim($this->getParam('consignee')),
				...
				'is_default' => intval($this->getParam('is_default'))
			);
		if(empty($param['consignee'])){
			$this->errorOutput('4001', '请填写收货人');
		}
		if(!api_CoreHelper::isMobile($param['mobile'])){
			$this->errorOutput('4001', '手机号不正确');
		}
		if(empty($param['province'])){
			$this->errorOutput('4001', '请选择城市');
		}
		if(empty($param['address'])){
			$this->errorOutput('4001', '请填写详细收货地址');
		}
		$res = api_User::addDeliveryAddress($param);
		if(isset($res['status']) && $res['status']=='0000'){
			$this->renderJson($res);
		}else{
			$msg = isset($res['msg']) ? $res['msg'] : '系统异常';
	 ...
	 * 编辑收货地址
	 */
	public function editDeliverAddressAction(){
		$param = array(
				'delivery_id' => intval($this->getParam('id')),
				'user_id' => $this->uid,
				'consignee' => trim($this->getParam('consignee')),
				'mobile' => trim($this->getParam('mobile')),
				'province' => trim($this->getParam('province')),
		...
		if($param['delivery_id']<1){
			$this->errorOutput('4001', '参数错误');
		}
		if(empty($param['consignee'])){
			$this->errorOutput('4001', '请填写收货人');
		}
		...
		$res = api_User::updateDeliveryAddress($param);
		if(isset($res['status']) && $res['status']=='0000'){
			$this->renderJson($res);
		}else{
			$msg = isset($res['msg']) ? $res['msg'] : '系统异常';
	 ...
	 * 删除收货地址
	 */
	public function removeDeliverAddressAction(){
		$param = array(
				'delivery_id' => intval($this->getParam('id')),
				'user_id' => $this->uid
			);
		...
		$res = api_User::removeDeliveryAddress($param);
		if(isset($res['status']) && $res['status']=='0000'){
			$this->renderJson($res);
		}else{
			$msg = isset($res['msg']) ? $res['msg'] : '系统异常';

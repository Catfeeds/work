...
<html>
  <head>
    ...
    <title>趣运动陪练 <?php echo $list['name']?></title>
  ...
  <body style="margin:0 auto;">
    <section id="loading" data-lock="0" class="loading">
      ...
      <div id="wxtitle" class="hide">趣运动陪练 <?php echo $list['name']?></div>
    ...
    <section class="yue-dt-main hide" style="padding-bottom: .5rem;">
      <section class="yue-dt-info se">
...
"><img style="opacity:0;height:100%;" data-origin=<?php echo $list['avatar_original']?> src=<?php echo $list['avatar']?>></li>
            <?php foreach ($list['gallery_list'] as $ksy =>$value) {?>
            <li style="
...
"><img style="opacity:0;height:100%;" data-origin=<?php echo $value['image_url']?> src=<?php echo $value['thumb_url']?>></li>
          <?php }?>
          <?php $count=count($list['gallery_list'])+1?>
          </ul><span>共<?php echo $count;?>张</span>
        </div>
        <div class="yue-dt-info-meta">
          <div class="yue-dt-info-cell"><span><?php echo $list['name']?></span>
         <?php if($list['gender']=='m'){ ?> <img src="/static/appwebview/yue/detail/images/male.png" class="yue-dt-info-icon-sex"><?php }?>
          ...
          <?php if($list['level']==3){ ?><img src="/static/appwebview/yue/detail/images/renzheng.png" class="yue-dt-info-icon-rz"><?php }?>
        ...
        <ul class="yue-dt-items">
          ...
          <p>运动经历：</p>
            <p>
            <?php if(empty($list['coach_experience'])){
                echo '未填写';
            }else{
            foreach ($list['coach_experience'] as $ksy =>$value) {
          ...
          <li>
            <p>常驻场馆：</p>
                <p>
               <?php if(empty($list['often_area_list'])){
      ...
      <section class="yue-dt-table se">
        ...
        <div id="yue-dt-1" class="yue-dt-proList">
           ...
           <?php foreach ($list['coach_projects'] as $key =>$value) {?>
          <div class="yue-list">
            <div class="yue-list-title"> 
             <span><?php echo $value['project_name'];?></span>
             <?php if($value['role_id']==1){ ?><img src="/static/appwebview/yue/detail/images/jiao.png"><?php }?>
             <?php if($value['role_id']==2){ ?><img src="/static/appwebview/yue/detail/images/pei.png"><?php }?>
             <span><?php echo $value['price'];?>元</span></div>
            <div class="yue-list-time">时长：<span><?php echo $value['minutes']/60;?>小时</span></div>
          </div>
           <?php }?>
        </div>
    ...
    <section class="yue-pop-sprite hide">
      <div class="yue-pop-box"><img src="/static/appwebview/yue/detail/images/close.png" class="yue-pop-close">
        ...
        <?php $Arr=explode('，',$list['coach_level']); foreach ($Arr as $key =>$value) { ?>
    ...
    <script src="/static/appwebview/yue/detail/js/detail.js?v=20151021c"></script>
    <script>

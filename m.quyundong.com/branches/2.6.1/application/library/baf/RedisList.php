...
class baf_RedisList
{
	protected $client;
	protected $name;
	protected $_data,$_count;
	function __construct($name=null){
		$this->client = baf_Redis::factory();
		$this->name = $name;
	}
	/**
	...
	public function add($item) {
		...
		if (!$this->client->rpush($this->name,$item)) {
			return false;
		}
		$this->_data = null;
		$this->_count = null;
	...
	public function remove($item) {
		...
		if (!$this->client->lrem($this->name,$item,1)) {
			return false;
		}
		$this->_data = null;
		$this->_count = null;
	...
	public function unshift($item) {
		...
		if (!$this->client->lpush($this->name,$item)) {
			return false;
		}
		$this->_data = null;
		$this->_count = null;
	...
	public function shift() {
		...
		$item = $this->client->lpop($this->name);
		$this->_data = null;
		$this->_count = null;
		return $item;
	}
	...
	public function pop() {
		...
		$item = $this->client->rpop($this->name);
		$this->_data = null;
		$this->_count = null;
		return $item;
	}
	...
	public function range($start = 0, $stop = -1) {
		...
		return $this->client->lrange($this->name, $start, $stop);
	}
	/**
	...
	public function trim($start, $stop) {
		...
		return $this->client->ltrim($this->name, $start, $stop) ? true : false;
	}
	...
	public function getCount() {
		if ($this->_count === null) {
			...
			$this->_count = (int) $this->client->lSize($this->name);
		}
		return $this->_count;
	}
	/**

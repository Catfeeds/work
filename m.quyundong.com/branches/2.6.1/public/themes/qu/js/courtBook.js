	...
	var utils = {
		...
		updateSum: function(){
            ...
            $.each(selectCourts2, function(i,v){
                ...
                str += '<span style="width:50%;display:inline-block;text-align:center;font-size:12px;">'+ contents[0] + ' ' + contents[3]+'</span>';
			});
            
			$('.total span').html(sum);
            $('.court-tips ul').html(cent);
            $('#select_court_info').html(str);
		},
		...
		$('.J_courts').on(touchType, '.court-detail', function(){
			...
			if(selectCourts[curGid]!=undefined){
				...
				if(minHour>1){
					...
					var col = el.parent();
					$('div[bind_id='+bindId+']').each(function(){
                ...
                if (selectNum > 4){
   				...
   				if(minHour>1){
					...
					for(var i=1;i<minHour;i++){
						if(!col.find('div').eq(currentIndex+i).hasClass('available')) enableDown = false;
						if(!col.find('div').eq(currentIndex-i).hasClass('available')) enableUp = false;
					}
					...
					for(i=1;i<minHour;i++){
						...
						var goodsId = col.find('div').eq(nextInd).attr('goodsid');
						selectCourts[goodsId] = parseInt(col.find('div').eq(nextInd).html());
		                selectCourts2[goodsId] = col.find('div').eq(nextInd).attr('course_content');
		                col.find('div').eq(nextInd).removeClass('available');
		   				col.find('div').eq(nextInd).addClass('selected');
		   				col.find('div').eq(nextInd).attr('bind_id',bindId);
					}
			...
			//如果最小起订时间大于1则自动选择
//			if(minHour>1 && isAuto==0 && el.hasClass('selected')){	
//				isAuto = 1;
//				var col = el.parent();
//				var firstIndex = 1;
//				var lastIndex = el.parent().find('div:last').index();
//				var currtIndex = el.index();			
//				//col.find('.court-detail').eq(currtIndex+1).click();
//				if(currtIndex>=1 && currtIndex<lastIndex){					
//					if(!col.find('div').eq(currtIndex+1).hasClass('selected')){
//						col.find('div').eq(currtIndex+1).click();
//					}else if(!col.find('div').eq(currtIndex-1).hasClass('selected')){
//						col.find('div').eq(currtIndex-1).click();
//					}
//				}else if(currtIndex==lastIndex){
//					if(!col.find('div').eq(currtIndex-1).hasClass('selected')){
//						col.find('div').eq(currtIndex-1).click();
//					}
//				}	
...
//			}
		...
		$('.J_submit').on(touchType, function(e){
			e.stopPropagation();
			e.preventDefault();
            var gids = [];
		    $.each(selectCourts, function(i,v){

...
$(function(){
    ...
    $(".li_balance_pay label").click(function(e){
        if($("#paylist-recharge-pay-i").prop("checked")){
            if($(".main").attr("data-password") == "0"){
                 e.stopPropagation();
                 e.preventDefault();
                // 没有设置支付密码
                return $(".nm-cover").removeClass("hide");
            }
        }
    })

    $('#paylist-recharge-pay-i').on('change',function(e){
        // e.stopPropagation();
        // e.preventDefault();

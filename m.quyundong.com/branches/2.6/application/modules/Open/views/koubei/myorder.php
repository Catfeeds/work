    ...
    <div class="main hide">
      <div class="myorder-index">
        ...
        <div class="myorder-index-card borderBottom1px">
          <div class="myorder-index-card-header borderBottom1px"><span><?php echo $v['order_title']?></span>
            <div class="myorder-index-card-header-right">
			<?php
			if ($v['order_status'] == 0){
			?>
                <a class="pay" href="pay?id=<?php echo $v['order_id']?>">立即支付</a>
			<?php
			}
			else {
			?>
                <span class="alert"><?php echo isset($orderStatus[$v['order_status']]) ? $orderStatus[$v['order_status']] : '';?></span>
            <?php
			}
			?>
            </div>
          ...
          <ul class="myorder-index-card-body">
            <li><span class="left">项目</span>&nbsp;<span class="right"><?php echo $v['cat_title']?></span></li>
            <li><span class="left">日期</span>&nbsp;<span class="right"><?php echo $v['start_date']?></span></li>
			<?php
            if(isset($v['courses_list']) && !empty($v['courses_list'])){
				$i = 0;
                foreach($v['courses_list'] as $courses){
            ?>
            ...
            -->
            <li><span class="left">验证码</span>&nbsp;<span class="right"><?php echo !empty($v['verification_code']) ? $v['verification_code']: '';?></span></li>
            <li><span class="left">订单号</span>&nbsp;<span class="right"><?php echo $v['order_sn']?></span></li>
          </ul>
        </div>
        <?php }} ?>
      </div>

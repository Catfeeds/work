...
class baf_Redis{
    ...
    public static function factory($server = 'master')
    {
        //基本验证
        $server = trim($server);
        if (empty($server) || !isset(Config::$redisServer[$server])) {
        ...
        //server配置
        $config = Config::$redisServer[$server];
		//var_dump($config);
        //参数
	    ...
	    baf_Common::log('redis_error','ERR','redis_error', json_encode($logArr));

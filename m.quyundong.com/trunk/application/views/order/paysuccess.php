	...
	<div class="main hide" >
		<?php if(in_array(UTM_SOURCE, Config::$source4Nothing)){
			...
			$this->display('public/public_top_202.php', array(
				'topTitle'=>'支付成功',
				'leftMenu' => isset($_SESSION['is_wap']) && $_SESSION['is_wap'] ? '<a href="javascript:history.go(-2);" class="J_back"><i class="icon-back"></i></a>' : '',
				'rightMenu' => isset($_SESSION['is_wap']) && $_SESSION['is_wap'] ? '<a class="pay-ok-right" style="color:#FFFFFF;" href="'.baf_CHtml::createUrl('/').'">首页</a>' : ''
			));
		}?>
            ...
            <!-- 支付成功 -->
				...
				<?php if(!in_array(UTM_SOURCE,ExtendConfig::$payokNoDeailView)) { ?>
					<!-- 查看订单详情 -->
					<div>
						<?php if (isset($order['status']) && $order['status'] == '0000' && ($order['order_status'] == '1' || $order['order_status'] == '6')) {?>
							<p>

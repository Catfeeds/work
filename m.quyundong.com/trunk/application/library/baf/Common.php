...
class baf_Common {
    ...
    */
    ...
    public static function verifyCode(){
        $code = isset($_REQUEST['vf_code']) ? $_REQUEST['vf_code'] : '';
        $verifyCode = isset($_SESSION['vf_code']) ? $_SESSION['vf_code'] : '';
        if(!$verifyCode || $verifyCode!=$code){
            ...
            baf_FileLog::log2File(json_encode($logData), 'verifyCode_error');
            return false;
        }
        return true;
    }
     ...
     * 激活图形验证码
     */
    public static function activeVerifyCode(){
        $_SESSION['active_vfcode'] = 1;
        return true;
    }
     ...
     */
    public static function isActiveVerifyCode(){
        if(!empty($_SESSION['active_vfcode'])){
            return true;
        }
        return false;
    }
     ...
     * @return [type] [description]
     */
    public static function releaseActiveVerifyCode(){
        unset($_SESSION['active_vfcode']);
    }
     ...
     * @return [type]       [description]
     */
    public static function smsSendTimeLimit($mobile){
        if(empty($mobile)) return false;
        $key = 'm.quyundong:sms_minlimit_'.$mobile;
     ...
     * @param [type] $time   [description]
     */
    public static function setSmsSendTimeLimit($mobile, $time){
        $key = 'm.quyundong:sms_minlimit_'.$mobile;
        $redis = baf_Redis::factory();
        return $redis->set($key, '1', $time);
    }
}

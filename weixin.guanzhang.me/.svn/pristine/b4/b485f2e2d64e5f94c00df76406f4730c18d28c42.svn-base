 ...
 * @type {{}}
 * 通过微信open_id->取商家id(7yundong)->取app_id(qyd_business)->转成商家中心的场馆ID(whale_business_db)
 * 旧版报表通过商家中心的场馆id取值
 */
    ...
    $async.waterfall([
            ...
            function (supplierIds, callback) {
                $qydBusiness.getListBySuppliersIds(supplierIds, function (err, appList) {
                    if (err) {
                        $log.write('getListBySuppliersIds', 0, '取商家APPId列表出错', {
                            err: err,
            ...
            function (appIds, callback) {
            	//通过app_id取商户中心的场馆id,报表场馆ID关联商户中心场馆ID
                $business.getVenueListByAppIds(appIds, function (err, venueList) {
                    if (err) {
                        $log.write('getVenueListByAppIds', 0, '取商家venueList列表出错', {
                            err: err,
                            appIds: appIds
                        }, 'error');

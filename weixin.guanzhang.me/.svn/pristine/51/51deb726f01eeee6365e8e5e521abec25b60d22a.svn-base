...
wxreport.getReportIndexData = function (req, res, next) {
    ...
    async.parallel({
        ...
        //趣运动场地总价改为结算价add by bumtime 20170317
        getQydCourtIncome: function (asyncCallback) {
            IncomeReport.getCourtIncomeByDate(params, asyncCallback);
        }
        ...
        log.write('wxreport', 0, 'fieldIncome:=', data.moneyGetData.fieldIncome , 'info');
        //加上趣运动的结算价 add by bumtime 20170321
        if (typeof results.getQydCourtIncome === 'object' && data.moneyGetData.fieldIncome >=0) {
            data.moneyGetData.fieldIncomeQyd = results.getQydCourtIncome.amount || 0;
            data.moneyGetData.fieldIncome += data.moneyGetData.fieldIncomeQyd;
        }
        ...
        //运营数据
        log.write('getReportIndexData', 0, 'results.getCourtTypeIncomes', results.getCourtTypeIncomes, 'info');
        if (typeof results.getCourtTypeIncomes === 'object') {
        	...
        	for (var i = 0; i < show_pay_type.length; i++) {
                ...
                if (results.getCourtTypeIncomes[pay_type] !== undefined && results.getCourtTypeIncomes[pay_type]) {
                    //该支付方式存在
                	//趣运动总价改为结算价add by bumtime 20170321
                	if(pay_type == 7){
                    	data.venueBusinessData[i].field = data.moneyGetData.fieldIncomeQyd;
                	...
                	//只有趣运动价时处理 add by bumtime 20170321
                	if(pay_type == 7){
                    	data.venueBusinessData[i].field = data.moneyGetData.fieldIncomeQyd;
                        //gift_amount += results.getCourtTypeIncomes[pay_type].gift_amount;
                        court_total += data.venueBusinessData[i].field;
        ...
        //有趣运动结算价，但是没有其他支付方式时 add by bumtime 20170321
        else{
        	log.write('getReportIndexData', 0, 'test', 'aa', 'info');
        	data.venueBusinessData[9].field = data.moneyGetData.fieldIncomeQyd;
            data.venueBusinessData[10].field += data.moneyGetData.fieldIncomeQyd;
        }

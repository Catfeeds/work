    ...
    this.init = function (limit, callback) {
        ...
        async.waterfall([
                ...
                conn.select("id,message").where("id>="+maxId+" AND status="+DEAL_STATUS).limit(limit)
                     //conn.select("id,message").where("id= 1246962").limit(limit)
        ...
        conn.where({
            ...
            if (!err && result.length > 0) {
                ...
                if (['110000', '110001', '110002'].indexOf(meta.SourceName) > -1) {
                    ...
                    var arry_version = meta.Version.split("-", 3);
                    var version_tip =  parseFloat(arry_version[2].substr(1,3));
                    value.card_id =  version_tip >= SOMS_SER ? value.CustomerId : '' ;

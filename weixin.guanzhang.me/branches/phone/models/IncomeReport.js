...
var incomeReport = function () {
        ...
        async.waterfall([
            ...
            function (data, callback) {
                ...
                conn.select('SUM(amount) AS amount, SUM(gift_amount) AS gift_amount')
                    .where('income_type=1 AND pay_type !=1 AND pay_type != 3 AND venue_id=' + params.venues_id + ' AND date=' + params.date)
    ...
    this.getNewTotalIncome = function (params, callback) {
        ...
        conn.select('date,SUM(amount) AS total')
            ...
            .where('venue_id=' + params.venues_id + " AND income_type !='3' AND pay_type != '3' AND date IN(" + params.dates.join(',') + ')')
            .group_by('date')
            .order_by('date')
            .get('income_report', function (err, results) {
                conn.disconnect();//释放
